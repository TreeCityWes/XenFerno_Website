...
error SparxFarmV2__NoLockOptionsProvided();
error SparxFarmV2__DurationsMustAscend();
error SparxFarmV2__BonusTooLow();
error SparxFarmV2__InvalidLockOptionIndex();
...
contract SparxFarmV2 is Ownable2Step, ReentrancyGuard, Pausable {
    ...
    // --- New Struct for Lock Options ---
    struct LockOption {
        uint256 duration;           // Lock duration in seconds
        uint256 bonusMultiplier;    // Bonus multiplier in basis points (10000 = 1x)
    }
    // --- End New Struct ---
    ...
    struct PoolInfo {
        ...
        // --- Added lock options array ---
        LockOption[] lockOptions;       // Array of available lock options for this pool
    }
    ...
    struct UserInfo {
        ...
        // --- Added chosen bonus multiplier ---
        uint256 chosenBonusMultiplier; // The bonus multiplier chosen by the user for their lock
    }
    ...
    // --- DEBUG EVENT --- 
    // event DebugCooldownCheck(uint256 blockTs, uint256 lastActionTs, uint256 cooldownVal, bool shouldRevert);
    // --- END DEBUG EVENT ---
    ...
    modifier noFlash(uint256 _pid) {
        ...
        if (user.lastActionBlock > 0 && 
            block.number < user.lastActionBlock + MIN_STAKE_BLOCKS) {
    ...
    modifier cooldownCheck(uint256 _pid) {
        ...
        // --- DEBUG LOGGING --- 
        // Emit an event instead of console.log
        // emit DebugCooldownCheck(block.timestamp, user.lastActionTimestamp, COOLDOWN, block.timestamp <= user.lastActionTimestamp + COOLDOWN);
        // --- END DEBUG LOGGING --- 
        ...
        _;
        // user.lastActionTimestamp = block.timestamp; // Timestamp updated in function body
    }
     ...
     * @param _options Array of lock duration and bonus multiplier options for this pool.
     */
    function addPool(
        uint256 _allocPoint,
        address _token,
        LockOption[] calldata _options // Changed input from lockDuration/bonusMultiplier
        ...
        // require(_lockBonusMultiplier >= 10000, "SparxFarmV2: Bonus must be >= 1x"); // Old validation removed

        // --- New Validation for Lock Options ---
        if (_options.length == 0) revert SparxFarmV2__NoLockOptionsProvided();
        if (_options[0].duration != 0) revert SparxFarmV2__DurationsMustAscend(); // Ensure first option is 0 duration for flexibility if needed later
        if (_options[0].bonusMultiplier < 10000) revert SparxFarmV2__BonusTooLow(); // Check first option bonus

        for (uint i = 1; i < _options.length; ++i) {
            if (_options[i].duration <= _options[i - 1].duration) revert SparxFarmV2__DurationsMustAscend();
            if (_options[i].bonusMultiplier < 10000) revert SparxFarmV2__BonusTooLow();
        }
        // --- End New Validation ---
        ...
        poolInfo.push(PoolInfo({
            ...
            accSparxPerShare: 0,
            supply: 0,
            lockOptions: new LockOption[](0) // Initialize empty array
            // lockDuration: _lockDuration,        // Removed
            // lockBonusMultiplier: _lockBonusMultiplier // Removed
        }));

        // Get index of the newly added pool
        uint256 newPoolIndex = poolInfo.length - 1;

        // Copy lock options from calldata to storage
        for (uint i = 0; i < _options.length; i++) {
            poolInfo[newPoolIndex].lockOptions.push(_options[i]);
        }
        ...
        emit PoolAdded(newPoolId, _token, _allocPoint); // Emit simpler event for now
    }
     ...
     * @param _optionIndex Index of the chosen lock option from poolInfo[_pid].lockOptions
     ...
     */
    function deposit(uint256 _pid, uint256 _amount, uint8 _optionIndex)
        external 
        whenNotPaused 
        nonReentrant 
        noFlash(_pid) 
        ...
        onlyWhenDepositsEnabled 
    {
        ...
        if (basePending > 0 && harvestingEnabled) {
            // Use the bonus multiplier stored in UserInfo
            uint256 reward = (basePending * user.chosenBonusMultiplier) / 10000;
            // Check final amount after potential rounding
            if (reward > 0) { 
        ...
        if (user.amount > 0 && block.timestamp < user.unlockTimestamp) {
            // User is still locked - ignore the new optionIndex
            // Only update amounts, leave unlockTimestamp and chosenBonusMultiplier untouched
            user.lastActionTimestamp = block.timestamp; 
            user.lastActionBlock = block.number; // Also update block here for consistency
        } else {
            // First deposit OR lock expired - apply the new choice
            // FIX: Only set lock info if amount > 0
            if (_amount > 0) {
                if (_optionIndex >= pool.lockOptions.length) revert SparxFarmV2__InvalidLockOptionIndex();
                LockOption memory chosenOption = pool.lockOptions[_optionIndex];
                user.unlockTimestamp = block.timestamp + chosenOption.duration;
                user.chosenBonusMultiplier = chosenOption.bonusMultiplier; // Store the chosen bonus
            } else {
                // If amount is 0, ensure no lock info is set/remains
                user.unlockTimestamp = 0;
                user.chosenBonusMultiplier = 0;
            }
        }
        // --- End lock behavior ---
        ...
        if (_amount > 0) {
            pool.supply += _amount;
            user.amount += _amount;
            // Transfer happens later (INTERACTIONS section)
        }
        // IMPORTANT: Update rewardDebt based on accumulator *after* potential reward minting
        user.rewardDebt = user.amount.mulDiv(pool.accSparxPerShare, ACC_PRECISION);
        user.lastActionBlock = block.number;
        user.lastActionTimestamp = block.timestamp;

        // Set or reset unlock timestamp - REMOVED, handled in lock behavior block above
        ...
        onlyWhenWithdrawalsEnabled 
    {
        ...
        // --- LOCK CHECK (uses user.unlockTimestamp, which is now set based on their choice) ---
        if (user.unlockTimestamp > block.timestamp) { // Check if lock is active
        ...
        _updatePool(pool); // Update pool state globally first
        
        // Calculate base pending rewards BEFORE state changes affect it
        uint256 basePending = pendingRewards(_pid, msg.sender);

        // 2. EFFECTS - Update ALL internal state before external calls
        if (_amount > 0) {
            user.amount -= _amount;
            pool.supply -= _amount;
        }
        
        // IMPORTANT: Update rewardDebt based on accumulator *after* pool update
        user.rewardDebt = user.amount.mulDiv(pool.accSparxPerShare, ACC_PRECISION);
        user.lastActionBlock = block.number;
        user.lastWithdrawTimestamp = block.timestamp;
        // Note: withdrawCooldownCheck modifier already updates lastActionTimestamp
        ...
        // Use the bonus multiplier stored in UserInfo
        if (basePending > 0 && harvestingEnabled) {
            uint256 finalReward = (basePending * user.chosenBonusMultiplier) / 10000;
            if (finalReward > 0) {
                _safeMint(msg.sender, finalReward); // Interaction 1: Mint rewards
        ...
        // Transfer LP tokens
        if (_amount > 0) {
            pool.lp.safeTransfer(msg.sender, _amount); // Interaction 2: Transfer LP
        }
        ...
        onlyWhenHarvestingEnabled 
    {
        ...
        // Update the pool first to account for rewards up to this block
        _updatePool(pool);

        // Calculate total pending rewards based on the updated pool state
        uint256 totalPending = user.amount.mulDiv(pool.accSparxPerShare, ACC_PRECISION);
        if (totalPending <= user.rewardDebt) { // Check if pending is zero or less than debt
            revert SparxFarmV2__NothingToHarvest();
        }

        // Calculate the actual base pending amount
        uint256 basePending = totalPending - user.rewardDebt;

        // Apply bonus multiplier - Use the bonus multiplier stored in UserInfo
        // Note: We apply bonus to basePending which includes rewards from the current block update
        uint256 finalReward = (basePending * user.chosenBonusMultiplier) / 10000;
        ...
        // IMPORTANT: Update rewardDebt based on the *updated* accumulator
        user.rewardDebt = user.amount.mulDiv(pool.accSparxPerShare, ACC_PRECISION);
        user.lastActionBlock = block.number;
        user.lastActionTimestamp = block.timestamp; // Also update timestamp
    ...
    /**
     * @notice Get lock info for a user's pool position and the available options for the pool.
     * @param _pid Pool ID
     * @param _user User address
     * @return chosenBonusMultiplier The bonus multiplier the user selected for their current lock (0 if no lock).
     * @return userUnlockTimestamp Timestamp when the user's current stake unlocks (0 if no lock or no deposit).
     * @return secondsRemaining Seconds remaining until unlock (0 if unlocked or no lock).
     * @return poolOptions The array of available LockOption structs for this pool.
     */
    function getUserStakeInfo(uint256 _pid, address _user) external view returns (
        uint256 chosenBonusMultiplier,
        uint256 userUnlockTimestamp,
        uint256 secondsRemaining,
        LockOption[] memory poolOptions 
    ) {
        if (_pid >= poolInfo.length) revert SparxFarmV2__InvalidPid(); // Changed error to match others
        PoolInfo storage pool = poolInfo[_pid];
        UserInfo storage user = userInfo[_pid][_user];

        // lockDuration = pool.lockDuration; // Removed
        // bonusMultiplier = pool.lockBonusMultiplier; // Removed
        chosenBonusMultiplier = user.chosenBonusMultiplier;
        userUnlockTimestamp = user.unlockTimestamp;

        if (block.timestamp < userUnlockTimestamp) {
            secondsRemaining = userUnlockTimestamp - block.timestamp;
        } else {
            secondsRemaining = 0;
        }

        poolOptions = pool.lockOptions; // Return the available options for the pool

        // return (
        //     lockDuration, // Removed
        //     bonusMultiplier, // Removed
        //     userUnlockTimestamp,
        //     secondsRemaining
        // );
    }
